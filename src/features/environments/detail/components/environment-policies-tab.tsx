...
interface EnvironmentPoliciesTabProps {
...
export function EnvironmentPoliciesTab({
  environment,
}: EnvironmentPoliciesTabProps) {
  ...
  const environmentPolicies = mockPolicies.filter(
    (p) =>
  ...
  const allPolicies = [...gatewayPolicies, ...environmentPolicies]
    ...
    <div className='space-y-4'>
      ...
      <Alert>
        ...
        <AlertDescription>
          ...
          environments) or at the environment level (specific to this
          environment).
        </AlertDescription>
      </Alert>
      ...
      {environmentPolicies.length > 0 ? (
        <Card>
          <CardHeader className='flex flex-row items-center gap-2'>
            <Globe className='h-5 w-5 text-muted-foreground' />
            <CardTitle>
              Environment-Specific Policies ({environmentPolicies.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className='mb-4 text-sm text-muted-foreground'>
              These policies are specific to this environment only.
            </p>
            <Table>
              <TableHeader>
                <TableRow>
        ...
        <Card>
          <CardHeader className='flex flex-row items-center gap-2'>
            <Globe className='h-5 w-5 text-muted-foreground' />
            <CardTitle>Environment-Specific Policies</CardTitle>
          </CardHeader>
          <CardContent>
            <p className='py-4 text-center text-sm text-muted-foreground'>
              No environment-specific policies configured. Using gateway-level
              policies only.
            </p>
          </CardContent>
        </Card>
